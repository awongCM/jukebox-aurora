...
router.route('/login')
  .post(function (req, res) {
    pm.login({ email: EMAIL, password: PASSWORD}, function (err, credentials) {
      ...
      pm.init({androidId: credentials.androidId , masterToken: credentials.masterToken}, function (err, data) {
        if(err) res.send({error: err});
        res.json({message : "Google Account Logged In!", accessToken: MASTER_TOKEN});
      });
    });
  });
