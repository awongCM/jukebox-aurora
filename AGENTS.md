...
- **Secrets:** Do not commit Spotify client IDs or other API keys. Copy `environment.example.ts` to **`environment.ts`** (gitignored) and add your client ID there only on your machine. For Cloud Agents, use Cursor Cloud Secrets (`SPOTIFY_CLIENT_ID`) if needed — never commit credentials.
